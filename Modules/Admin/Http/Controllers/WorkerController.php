...
use Admin\Http\Resources\Worker\WorkerCollection;
use Admin\Http\Resources\Worker\WorkerResource;
...
class WorkerController extends Controller
{
    ...
    /**
     * @OA\Get(
     *       path="/api/v1/admin/worker/list",
     *       tags={"Admin|Рабочие"},
     *       security={{"bearerAuth":{}}},
     *       summary="Список рабочих",
     *       operationId="AdminGetWorkers",
     *       @OA\Parameter(
     *           name="filter[id]",
     *           in="query",
     ...
     */
    public function list(ListPersonRequest $request): ApiErrorResponse|WorkerCollection
    {
        try {
            $dataApp = $request->validated();
            return new WorkerCollection($this->personService->list($dataApp, $this->type));
        } catch (Throwable $e) {
    ...
    public function show(int $id): ApiSuccessResponse
    {
        return new ApiSuccessResponse(
            new WorkerResource($this->personService->show($id, $this->type)),
            ['message' => 'Ok'],
    ...
    /**
     * @OA\Get(
     *      path="/api/v1/admin/worker/export",
     *      operationId="exportWorkers",
     *      security={{"bearerAuth":{}}},
     *      tags={"Admin|Рабочие"},
     *      summary="Экспорт списка рабочих",
     *           @OA\Parameter(
     *          name="filter[from]",
     *          in="query",
     ...
     *                 property="data",
     *                 type="string",
     *                 example="C:\\xampp\\htdocs\\newapiartrussiafair\\storage\\/app//workers/workers-2023-12-14 07:56:25.xlsx"
     *                 ),
     *               @OA\Property(property="metadata", type="object",
     *                @OA\Property(property="message", type="string", example="Ok"),
     *               ),
    ...
    {
        try {
            $fileName = '/workers/workers-' .date('Y-m-d_H-i-s'). '.xlsx';
            Excel::store(
                new WorkerExport($this->commonService->dateInterval($request->validated())),
                $fileName
            );
