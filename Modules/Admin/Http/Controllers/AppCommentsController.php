...
use Admin\Services\AppCommentService;
...
class AppCommentsController extends Controller
{
    ...
    public AppCommentService $commentService;

    public function __construct(AppCommentService $commentService)
    {
    ...
    /**
     * @OA\Post(
     *      path="/api/v1/admin/applications/{id}/comment",
     *      operationId="createAppManagerComment",
     *      security={{"bearerAuth":{}}},
     *      tags={"Admin|Заявки|Комментарии менеджера"},
     ...
     *
     */
    public function store(int $id, CommentUserApplicationRequest $request): ApiSuccessResponse|ApiErrorResponse
    {
        try {
            $data = $request->all();
            $data['user_application_id'] = $id;
            return new ApiSuccessResponse(
                new  CommentApplicationResource($this->commentService->create($data)),
     ...
     * @OA\Get(
     *      path="/api/v1/admin/applications/{id}/comment",
     *      operationId="getAppAllManagerComment",
     *      security={{"bearerAuth":{}}},
     *      tags={"Admin|Заявки|Комментарии менеджера"},
    ...
    {
        return new ApiSuccessResponse(
            new  CommentApplicationCollection($this->commentService->list($id, $request)),
            ['message' => 'Ok'],
            ResponseAlias::HTTP_OK
        );
    }
    ...
    /**
     * @OA\Get(
     *      path="/api/v1/admin/applications/{id}/comment/{comment_id}",
     *      operationId="getAppManagerComment",
     *      security={{"bearerAuth":{}}},
     *      tags={"Admin|Заявки|Комментарии менеджера"},
    ...
    {
        return new ApiSuccessResponse(
            new  CommentApplicationResource($this->commentService->show($comment_id, $request)),
            ['message' => 'Ok'],
            ResponseAlias::HTTP_OK
        );
    }
     ...
     * @OA\Patch(
     *      path="/api/v1/admin/applications/{id}/comment/{comment_id}",
     *      operationId="editAppManagerComment",
     *      security={{"bearerAuth":{}}},
     *      tags={"Admin|Заявки|Комментарии менеджера"},
    ...
    ): ApiSuccessResponse|ApiErrorResponse
    {
        try {
            $data = $request->all();
            return new ApiSuccessResponse(
                new  CommentApplicationResource($this->commentService->update($comment_id, $data)),
     ...
     * @OA\Delete(
     *      path="/api/v1/admin/applications/{id}/comment/{comment_id}",
     *      operationId="deleteAppManagerComment",
     *      security={{"bearerAuth":{}}},
     *      tags={"Admin|Заявки|Комментарии менеджера"},
    ...
    {
        try {
            $message = $this->commentService->delete($id, $commentId) ? 'Комментарий успешно удалён' : '';
